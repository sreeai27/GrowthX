...
import { getStudioContactGateway, type RevealReason } from "./studio-contact-gateway";
...
export async function revealStudioContact(_state: RevealState, data: FormData): Promise<RevealState> {
  ...
  try {
    const value = await getStudioContactGateway().revealContact({ ...actor, contactId: String(data.get("contactId") ?? ""), reason });
