...
import { reviewerCookieName, reviewerIsAuthorised } from "./auth";
import { promoteExampleCorrection, runSuite, signInReviewer } from "./actions";

export default async function EvaluationsPage() {
  const authorised = reviewerIsAuthorised((await cookies()).get(reviewerCookieName)?.value, process.env.STUDIO_REVIEWER_TOKEN);
  if (!authorised) return <main className="placeholder"><p className="eyebrow">Hunar Studio</p><h1>Reviewer access required</h1><p>Evaluation evidence is limited to authorised reviewers.</p><form action={signInReviewer}><label>Reviewer token<input name="reviewerToken" type="password" required /></label><button className="button button-navy" type="submit">Open evaluations</button></form></main>;
