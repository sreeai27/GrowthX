...
export default async function WorkerConfirmationStatusPage({
  ...
  const copy =
    result.confirmation.status === "PENDING" &&
    result.confirmation.requestConfirmed
      ? {
  ...
  if (receipt) {
    ...
    }).format(snapshot.priceDeltaMinor / 100);
    return (
      <main className="incident-shell confirmation-shell">
        <header className="incident-topbar">
          ...
          <span className="incident-step">REVISED AGREEMENT</span>
        </header>
        <article className="confirmation-outcome revised-agreement">
          <p className="eyebrow">Shared result · साझा नतीजा</p>
          ...
          <p className="revised-impact">
            +{snapshot.durationDeltaMinutes} minutes · {price}
          </p>
          <section
          ...
          >
            ...
            <div>
              ...
              <h2>{receipt.externalActionId}</h2>
              <p>{new Date(receipt.executedAt).toLocaleString("en-IN")}</p>
              <p className="source-version">
                Booking version {receipt.previousBookingVersion} →{" "}
                {receipt.resultingBookingVersion}
              </p>
            </div>
          </section>
          <a
            className="button button-teal"
            href={`/worker/incidents/${incidentId}/completion`}
          >
            Record completed work · पूरा काम दर्ज करें
          </a>
        </article>
      </main>
    );
  ...
  if (
    ...
    execution?.status === "PERMANENT_FAILED" ||
    ...
    return (
      <main className="incident-shell confirmation-shell">
        <article className="confirmation-outcome terminal-failure">
          <p className="eyebrow">Action status · कार्रवाई की स्थिति</p>
          <h1>
            {reconciliation
              ? "Connector result needs reconciliation"
              : execution.status === "PENDING"
                ? "Booking update in progress"
                : retryable
          ...
          <p lang="hi">
            {reconciliation
              ? "कनेक्टर नतीजे की जाँच ज़रूरी है"
              : "बुकिंग में बदलाव अभी दर्ज नहीं हुआ"}
          </p>
          <p>
            {reconciliation
              ? "The connector reported success while the booking record changed. Do not repeat the action or promise either booking version; escalate to support."
              : retryable
                ? "Ask the customer to retry from the same secure link."
                : execution.status === "PENDING"
                  ? "Refresh to check the stored connector result."
                  : "Continue with the original booking and contact support."}
          </p>
        </article>
      </main>
    );
  ...
  return (
    <main className="incident-shell confirmation-shell">
      ...
      <article className="confirmation-outcome">
        ...
        <p>{copy.body}</p>
        <span
          className={`confirmation-state state-${result.confirmation.status.toLowerCase()}`}
        >
      ...
      {customerHref && result.confirmation.status === "PENDING" ? (
        <section
          className="confirmation-link-card"
        ...
        >
          ...
          <p className="confirmation-expiry">
            Expires{" "}
            {new Date(result.confirmation.expiresAt).toLocaleTimeString(
              "en-IN",
              {
                hour: "2-digit",
                minute: "2-digit",
